...
Until we complete the transition to the new ABL schema we are maintaining two versions of the ABL file:

* `approved-books.json` - The first version of the ABL which will be deprecated
* `approved-book-list.json` - The new version of the ABL which will incorporate books from both archive and git in one file

During this transition period, CMs will update `approved-books.json` and (if needed) `book-slugs.json`. Any discrepancies that result relative to the `approved-book-list.json` should cause a CI failure at which time a new `approved-book-list.json` can be generated by a CE developer and added to the PR using the `convert_abl.py` script:

```bash

$ python convert_abl.py approved-books.json book-slugs.json approved-book-list.json
```

### [Old ABL](./approved-books.json)

The file [approved-books.json](./approved-books.json) is hand-editable and contain the said list of books according to the following pattern:
...
### [New ABL](./approved-book-list.json)
